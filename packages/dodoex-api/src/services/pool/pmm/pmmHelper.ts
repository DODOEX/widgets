  ...
  public QuerySellBase(amount: BigNumber, state: PMMState): BigNumber {
    try {
      ...
      if (state.RStatus === RStatusOne) {
        ...
        const backToOnePayBase: BigNumber = state.B0.minus(state.B);
        const backToOneReceiveQuote: BigNumber = state.Q.minus(state.Q0);
      ...
      const lpFee = result.multipliedBy(state.lpFeeRate);
  ...
  public QuerySellQuote(amount: BigNumber, state: PMMState): BigNumber {
    try {
      ...
      if (state.RStatus === RStatusOne) {
        ...
        const backToOneReceiveBase = state.B.minus(state.B0);
        const backToOnePayQuote = state.Q0.minus(state.Q);
...
export const solveQuadraticFunctionForTrade = (
  ...
  if (k.eq(1)) {
    const tmp = i.multipliedBy(delta).multipliedBy(V1).div(V0.multipliedBy(V0));
...
export const solveQuadraticFunctionForTarget = (
  ...
  const sqrt = k
    ...
    .plus(1)
    .sqrt();
  const premium = sqrt.minus(1).div(k.multipliedBy(2)).plus(1);
