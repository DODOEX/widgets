...
export const TableList = ({
  ...
  return (
    <LiquidityTable>
      <Box component="thead">
        <Box component="tr">
          {inSelected && <Box component="th" />}
          <Box component="th">
            <Trans>Lock</Trans>
          </Box>
          <Box component="th" sx={{ whiteSpace: 'nowrap' }}>
  ...
  return (
    ...
    >
      {inSelected && !!data && (
        ...
        >
          <Checkbox
            disabled={selectedId.length > 1 && !checked}
            checked={checked}
            onChange={(evt: React.ChangeEvent<HTMLInputElement>) => {
              const { checked } = evt.target;
              if (checked) {
                setSelectedId((prev) => {
                  if (prev.length < 2 && !prev.includes(data.tokenId)) {
                    return [...prev, data.tokenId];
                  }
                  return prev;
                });
              } else {
                setSelectedId((prev) => {
                  const index = prev.indexOf(data.tokenId);
                  if (index > -1) {
                    const result = [...prev];
      ...
      >
        ...
        >
          {/* {formatPercentageNumber({ input: 0.0211307 })} */}
        </LoadingSkeleton>
      </Box>
      <Box component="td">
        <LoadingSkeleton
          component="span"
          loading={!data?.value || !token}
          loadingProps={{
      ...
      >
        {data?.lockedEnd ? (
          <Tooltip title={getUnlockTimeText(data.lockedEnd)}>
            <Box>{getUnlockTimeTextShort(data?.lockedEnd)}</Box>
          </Tooltip>
        ) : null}
      </Box>
      <Box component="td">
